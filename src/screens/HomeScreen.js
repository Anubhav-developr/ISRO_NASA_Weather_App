...
export default function HomeScreen() {
  ...
  const applyResolvedLocation = (location, clearSearchList = true) => {
    if (!location) return;
    ...
    try {
      ...
      try {
        ...
        if (first) {
          ...
          if (parts.length) {
            setEarthLocationName(`${parts.join(", ")} (Current Location)`);
          }
        }
      } catch {
  ...
  const createAndShareForecastPdf = useCallback(async () => {
    ...
    if (Platform.OS === "web") {
      setUiError("PDF share feature Android/iOS Expo Go par best work karta hai.");
      return;
    }
    ...
    try {
      ...
      if (!canShare) {
        setUiError(`PDF generate ho gaya, lekin sharing available nahi hai (${file.uri}).`);
        return;
      }

      await Sharing.shareAsync(file.uri, {
        mimeType: "application/pdf",
        dialogTitle: "Share Weather Forecast PDF",
        UTI: ".pdf",
      });
    } catch (error) {
